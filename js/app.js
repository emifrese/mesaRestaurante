...
function guardarCliente() {
    ...
    if(camposVacios) {
        ...
        if(!existeAlerta){// console.log('Si hay al menos un campo vacio');
        const alerta = document.createElement('div');
        alerta.classList.add('invalid-feedback', 'd-block', 'text-center');
        alerta.textContent = 'Todos los campos son obligatorios';
        document.querySelector('.modal-body form').appendChild(alerta);

        // Elimina nuestra alerta
        setTimeout( () => {
            alerta.remove();
        }, 2000);}
...
function obtenerPlatillos() {
    ...
    const url = 'db.json'; //Lo dejo asi para subirlo a github
...
function mostrarPlatillos(platillos) {
    ...
    platillos.forEach( platillo => { 
        ...
        inputCantidad.onchange = function() {
            ...
            agregarPlatillo({...platillo, cantidad});// de esta forma enviamos un objeto, no por separado
        }// De esta forma no manda a llamar la funcion, y a la vez mandamos el id
...
function agregarPlatillo(producto) {
    ...
    if(producto.cantidad > 0) {
        ...
        if( pedido.some( articulo => articulo.id === producto.id)){
            ...
            // cliente.pedido[producto.id-1].cantidad = producto.cantidad; Este metodo lo probe yo, funcionaba
